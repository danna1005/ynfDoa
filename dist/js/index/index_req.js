...
2016-12-28 拓凡
*/
var doaVersion = "beta-1.0";
var curAppClass = "";
var storage = window.localStorage;
var authEkp = "";
$(document).ready(function(){
	
	doaVersion = $("body").data("version");
...
function fetchAppAndNews()
{
	...
	var lastDateTime = storage.getItem("lastDateTime"),
		appCategory  = storage.getItem("appCategory"),
...
function hadleIndexDetails()
{
	...
	$("#newsMoreHref").attr("href",news_more_href);
	//钉邮和考勤的URL 启用缓存的URL
...
function geneAppSpan(item)
{
	...
	var html='<span class="col-25">\
	            <a href="'+item.url+'" class="a1 '+needOpenLink+'" data-anchor="'+item.icon+'" onclick="aListClick(this)">\
...
function improveAppIconUrl(list) {
    ...
    for (var i = 0; i < list.length; i++) 
    {
        ...
        if(list[i].icon=="more.png")
        {
        	...
        	list[i].urlType = 3;
//        	list[i].extendPara='{"needOpenLink":true}';
		...
		if(noError(r)){
			if(r.data.length > 0)
			{
				...
				for(var i=0;i<r.data.length;i++)
				{
					...
					html += '<tr><td><a href="'+item.url+'">'+item.title+'<a></td></tr>';
				}
