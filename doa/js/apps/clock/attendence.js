	...
	$("body").on("click",".ignoreBtn",function(){
		...
		commonGet(url,null,function(r){
			if(r.data && r.data.success == true){
				btn.closest(".item-after").html("<span class='ignore-text'>已忽略</span>");
			}else{
				var eMsg = r.errMsg;
...
{
	//用本地存储刷新一下考勤打卡链接
	var attenUrl = window.localStorage.getItem("attenUrl");
	if(attenUrl && attenUrl!="" && attenUrl!=''){
		$("a.attenAppUrl").attr("href",attenUrl);
	}
...
{	
	...
	if(status == 0)
	{
		statusHtml =  '<a class="ignore dealBtn" href="#" data-get-href="' + dealUrl + '" data-approval-href="'+approvalUrl+'">处理</a>\
   	   					<a href="#" class="ignore ignoreBtn" data-get-href="' + ignoreUrl + '">忽略</a>';
	}
	else if(status == 1)
	{
		statusHtml =  '<span class="ignore-text">已忽略</span>';
	}
	else if(status == 2)
	{
		statusHtml =  '<span class="ignore-text">已处理</span>';
	}
