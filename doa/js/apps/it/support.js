...
//		    	$('.popup').css('top',winHeight-290);
 ...
 function setUserLocation(value,i){	
	...
	//问题类型获取
function getType(){
	url = makeUrl("third/doa/doa_problem_type/doaProblemType.do?method=jsonData&parentId=159af2fcfb307d635cbf36e45f0822ab");
	commonGet(url,null,function(data) {					
		 ...
		 $.each(data.data, function(i, item) {
			...
			if(i==0){
				...
				parentId = item.fdId;
				getType1(item.fdId);
				$("#question").append('<option calss="option" value="'+id+'">'+item.name+'</option>');
			}else{
				$("#question").append('<option calss="option" value="'+id+'">'+item.name+'</option>');
			}	    	
		 });
...
//问题类型二级目录获取
function getType1(parentId1){
	if(parentId1!=null){
		url = makeUrl("third/doa/doa_problem_type/doaProblemType.do?method=jsonData&parentId="+parentId1);
		commonGet(url,null,function(data) {					
			 $("#question1").html(""); 
			 $("#question1").append('<option calss="option" value="1">请选择</option>');
			 $.each(data.data, function(i, item) {
					typeId = item.fdId;
					$("#question1").append('<option calss="option" value="'+typeId+'">'+item.name+'</option>');
			 });
	    }, false);	
	}
...
function func(obj){
	 ...
	 var vs =  obj.options[index].value;
	 getType1(vs);
}
